...
<body>
  ...
  <div id="app">
    <div class="main-wrapper main-wrapper-1">
      ...
      <nav class="navbar navbar-expand-lg main-navbar sticky">
        ...
        <li class="">
            <div class="mr-5">
            <span>Total Tithe For Current Month</span>
  ...
  <script>
    // displaying members when screen loads
    $.ajax({
      type:'POST',
      url: "ajax.php",
      data:"show=all",
...
// Function for inserting amount into database
    $("#record").click(function(){
      ...
      $.ajax({
        type:"POST",
        url: "ajax.php",
        data: {amember: name, amount: amount},
        success: function(){
          alert("success");
     ...
     var memdetails = document.getElementById('member_welfare');
      ...
      // sum up total tithes of all members for the month
      function sumAll(res){
        const d = new Date();
        ...
        $.ajax({
          ...
          success: function(data, status){
               ...
               for(let i = 0; i < res.length;i++){
                   const arr = res[i]['date'].split('/');
                   const amnt = parseFloat(res[i]['amount']);
                   if(arr[1] == month){
                       totalofmonth += amnt;
                   }
               }
               var floatTom = parseFloat(totalofmonth);
               $('#currentbal').text("₵ "+floatTom);
          }
      })
...
// Function for showing one member records
   function member(member){
     ...
     var tbody = document.getElementById('membertithes');
      ...
      $.ajax({
       ...
       success: function(data, status){
        $('#membrs').empty();
         ...
         const resultsQty = Math.ceil(searchr.length/3);
         ...
         var displayed = 0;
            for(let i = 0; i<4;i++){
              if(displayed ===nor-1){
                ...
                row.append(td);
                //  console.log(searchr[displayed]['fullname']);
                //  console.log('last');
                break;
              }else{
                var td = $("<td onclick='member(this.innerHTML)'></td>").text(searchr[displayed]['fullname']);
